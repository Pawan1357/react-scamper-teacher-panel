...
export const useSetupPassword = () => {
  ...
  const onSubmit = (values: { new_password: string; password: string }) => {
    if (!token) {
      showToaster('error', 'Setup token is missing.');
      return;
    }

    const payload: IResetPasswordReq = { reset_token: token, password: values.new_password };
